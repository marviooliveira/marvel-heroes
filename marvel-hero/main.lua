...
--------carrega audio de transicao--------------------
	...
	transicao : setLooping(false)
	love.audio.setVolume(10)
	toca_transicao=false
---------------------------fim menu----------------------------
...
----------------------ainda do menu, aqui carrega play, exit e instrucoes------------
	...
	----------exit e play again para fim jogo-------------
	exit={}
	...
	playagain={}
		playagain.img=love.graphics.newImage("img/playagain.png")
		playagain.x=350
		playagain.y=410
		playagain.w=playagain.img:getWidth()
		playagain.h=playagain.img:getHeight()
...
---------teste--------
	if fimjogo then
		if   placar.iron>placar.spider then
			ironcampeao=true
		end
		if  placar.iron<placar.spider then
			spidercampeao=true
...
function drawinstrucoes( )
	...
	if startinstrucoes and startmenu==false then
...
----------------------------------AQUI COMECO A DESENHR gols dos dois---------------------
		--love.graphics.setColor(255,255,255)
		love.graphics.print(placar.iron,405,37,0,3,4)
		--love.graphics.setColor(255,255,255)
		love.graphics.print(placar.spider,553,37,0,3,4)
-------------------tempo do jogo---------------------
		love.graphics.setFont(fonte)
		love.graphics.setColor(255,255,255)			
		love.graphics.print( tempo, 450,20,0,2.5,2.5)
		love.graphics.setColor( 255 , 255 , 255)		
-------------------------------------------------------
end
function drawironcampeao()
	love.graphics.draw(ironcampeaoimg,0,0)
	love.graphics.draw(exit.img, exit.x,exit.y)
end
function drawspidercampeao()
...
--------------COMANDOS DE CLICAR DENTRO DO MENU-------------------
	...
	if  startmenu and button==1 and px>=play.x and px < play.x + play.w and py >=play.y and py < play.y + play.h then
...
------------------- CLIQUE DENTRO DE INSTRUCOES---------------
	if  startinstrucoes and button==1 and px>=skip.x and px < skip.x + skip.w and py >=skip.y and py < skip.y + skip.h then
		startcenario1=true
		startinstrucoes=false
		transicao:play()
	end
------------clique dentro do fim do jogo--------------------
	...
	--------------PARA O PLAY AGAIN------------------------
	if fimjogo and button==1 and px>=playagain.x and px < playagain.x + playagain.w and py >=playagain.y and py < playagain.y + playagain.h then
		fimjogo=false 
		startcenario1=true
	end
end
